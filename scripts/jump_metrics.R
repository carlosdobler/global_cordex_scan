...
# land mask
{
  ...
  rm(rast_reference_0.05, rast_reference_0.2, box_reference)
}
...
map(vars, function(v){                                                                              # ***************************
  ...
  # select_chunks <- c(12,4,3,2,10,9,8,28,27,25,19)
  ...
  pwalk(st_drop_geometry(chunks_ind), function(lon_ch, lat_ch, r){
    ...
    # st_drop_geometry(chunks_ind)$lon_ch[12] -> lon_ch
    # st_drop_geometry(chunks_ind)$lat_ch[12] -> lat_ch
    # st_drop_geometry(chunks_ind)$r[12] -> r
  ...
  tb_files$model %>% 
    ...
    walk(function(mod_){
      ...
      saveRDS(s_mos, str_glue("output/{dom}_{v}_issues_{mod_}_v5.rds"))
