...
func_jump_4 <- function(x, dates){
  ...
  # dates <- d
  # dates <- seq(as_date("19700101"), as_date("21001201"), by = "1 month")[1:length(x)]
  ...
  if(count_na == length(x)){
    ...
    if(bkpt == length(x_trend_cut) | bkpt < which(dates == "1980-01-01")){
      ...
      zoo::rollapply(x[1:bkpt],
        ...
        {
          cbind(imputeTS::na_locf(.[,1]),
                imputeTS::na_locf(.[,2]))
        } -> x_outliers_1 
      
      w <- 15*12+1
      zoo::rollapply(x[(bkpt+1):length(x)],
                     FUN = function(y){
    ...
    if(bkpt_v > which(dates == "2030-01-01")){
      ...
      zoo::rollapply(x_detrend,
        ...
        imputeTS::na_interpolation() -> x_var
